...
var ConsoleLogViewer = (function () {
  ...
  ConsoleLogViewer.LOG_ENABLED = CONSOLE;
  ...
  ConsoleLogViewer.prototype.updateLog = function () {
    if (!ConsoleLogViewer.IS_MINIMIZED) {
      document.getElementById("debug_console_messages").innerHTML =
        _items.join("<br>");
    } else {
      var minimized = [];
      for (
        var i = Math.max(0, _items.length - 3), leni = _items.length;
        i < leni;
        i++
      )
  ...
  ConsoleLogViewer.prototype.applyCustomSettings = function () {
    ...
    if (
      url.indexOf("//try.haxe.org") > -1 ||
      url.indexOf("try-haxe.mrcdk.com") > -1
    ) {
      ...
      document.getElementById("debug_console_close_button").style.display =
        "none";
      document.getElementById("debug_console_minimize_button").style.display =
        "none";
      document.getElementById("debug_console_position_button").style.display =
        "none";
      document.getElementById("debug_console_pause_button").style.display =
        "none";
    }
  };
  ...
  ConsoleLogViewer.prototype.overwrite = function () {
    ...
    if (original.console.log)
      console.log = function () {
        self.log(
          self.flatten(Array.prototype.slice.call(arguments).join(",")),
          "log-normal",
          true
        );
    ...
    window.onerror = function (message, url, lineNumber) {
      self.log(
        [
          message,
          "<a target='_blank' onclick='javascript:DebugSource.show(this.href, this.parentNode.innerText);return false' href='" +
            url +
            "#" +
            lineNumber +
            "'>" +
  ...
  ConsoleLogViewer.prototype.addDivs = function (self) {
    ...
    var html =
      '<a href="#close" id="debug_console_close_button" class="log-button"></a>';
    html +=
      '<a href="#minimize" id="debug_console_minimize_button" class="log-button"></a>';
    html +=
      '<a href="#position" id="debug_console_position_button" class="log-button"></a>';
    html +=
      '<a href="#pause" id="debug_console_pause_button" class="log-button"></a>';
    ...
    document.getElementById("debug_console_position_button").addEventListener(
      "click",
      function (e) {
        ConsoleLogViewer.ALIGNMENT =
          ConsoleLogViewer.ALIGNMENT == "top" ? "bottom" : "top";
  ...
  ConsoleLogViewer.prototype.setClosed = function (value) {
    // true | false
    ConsoleLogViewer.IS_CLOSED = value;

    document.getElementById("debug_console_close_button").innerHTML = !value
      ? "x"
      : "&Xi;";
    document.getElementById("debug_console").style.background = !value
      ? "rgba(0, 0, 0, .75)"
      : "transparent";
    document.getElementById("debug_console_messages").style.display = !value
      ? "block"
      : "none";
    document.getElementById("debug_console_minimize_button").style.display =
      !value ? "inline" : "none";
    document.getElementById("debug_console_position_button").style.display =
      !value ? "inline" : "none";
    document.getElementById("debug_console_pause_button").style.display = !value
      ? "inline"
      : "none";
  };

  ConsoleLogViewer.prototype.setLogEnabled = function (value) {
    // true | false
  ...
  ConsoleLogViewer.prototype.setMinimized = function (value) {
    // true | false
  ...
  ConsoleLogViewer.prototype.alignTo = function (value) {
    // top | bottom
    ConsoleLogViewer.ALIGNMENT = value;
    document.getElementById("debug_console").className =
      value == "bottom" ? "bottom-aligned" : "top-aligned";
    document.getElementById("debug_console_position_button").innerHTML =
      value == "bottom" ? "&uarr;" : "&darr;";
  };

  ConsoleLogViewer.prototype.addCSS = function () {
    var css =
      "#debug_console { background: rgba(0,0,0,.75); position:fixed; padding:0; margin:0; z-index:12834567; box-sizing:border-box; pointer-events:none; text-align:left; text-transform:none; }";
    css +=
      "#debug_console, #debug_console * { font: 12px sans-serif!important;  }";
    css +=
      "#debug_console_messages { background:transparent;pointer-events:none; }";
    css +=
      "#debug_console_button { border:1px solid #fff; position:absolute; z-index:2;  }";
    ...
    css +=
      "#debug_console a.log-button {font: bold 12px sans-serif!important; pointer-events:all; text-align:center; text-decoration:none; border:1px solid #999; background:#333; color:#fff; width:16px; height:16px; padding:5px; margin:1px; display:block; float:right; box-sizing: content-box; }";
    ...
    http.onData = function (data) {
      ...
      while (_g_i < _g_items.length) {
        ...
        if (obj.index >= startLineNumber && obj.index <= endLineNumber) {
          content =
            '<div class="highlighted-line" id="highlight"><div class="debug-error"><strong>Error:</strong><em>' +
            message +
            "</em></div>" +
            content +
            "</div>";
      ...
      case "object":
        if (o instanceof Array) {
          if (o.__enum__) {
            ...
            while (_g1 < _g) {
              var i = _g1++;
              if (i != 2) str += "," + js_Boot.__string_rec(o[i], s);
              else str += js_Boot.__string_rec(o[i], s);
            }
            return str + ")";
          }
          var l = o.length;
